...
Phase 3 の現状報告をもとに設計を考える。設計を考える過程で以下の観点を確認し、該当すれば先に Phase 4.5 へ進む:

> **Phase 4.5 への移行条件**: `doc/Architecture.md` / `doc/DetailDesign.md` / `doc/Spec.md` に変更を加えることで、今回の実装をより少ないコード変更量で実現できる場合。

該当しない場合は、以下のフォーマットで設計を提示してユーザーの承認を得る:
...
Phase 4 で設計を考えた結果、ドキュメントを変更すればより少ないコード変更量で実現できると判断した場合にこのフェーズへ進む。

以下のフォーマットで提案しユーザーの了承を得る:
...
**y の場合**: ドキュメントを更新してから Phase 4 の設計提示に戻る。
**N / 後回しの場合**: 改善なしで Phase 4 の設計提示へ進む。
