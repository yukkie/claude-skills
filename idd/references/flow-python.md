...
## Phase 4.5 — 既存設計の改善提案（任意）

Phase 4 で設計を考えた結果、以下のいずれかに該当する場合にこのフェーズへ進む。該当しない場合はそのまま Phase 5 へ。

### トリガー条件

1. **設計改善**: 今回の Issue の実装をよりシンプルに実現できる設計変更が `doc/Architecture.md` / `doc/DetailDesign.md` に対して必要
2. **記載漏れ**: ドキュメントが実装の実態と乖離していて、そのまま実装すると混乱の原因になる
3. **実装中の気づき**: Phase 6 実装中に既存コードとの一貫性向上やシンプル化に気づいた場合も、その場で1〜2文で提案する（実装は承認後）

### 提案の優先順位

1. **必須で提案**: 今回の Issue の実装をよりシンプルに実現できる設計改善
2. **任意で提案**: Issue に記載されている将来の拡張・変更可能性に関連する設計改善
3. **スキップ**: Issue と無関係な一般的な拡張性・移植性の改善
...
**承認の場合**: ドキュメントを更新してから Phase 5 へ進む。
**N / 後回しの場合**: そのまま Phase 5 へ進む（設計変更なし）。
