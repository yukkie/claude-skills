...
### Contract テスト対象の判定（JS 固有）

以下は「見た目は些末だが契約である」ため、**実装より先にテストを書いて `to`／遷移先を縛る**。「Link を足すだけ」と軽視してテストファーストを飛ばさない:

- **要素に `Link`/`to` を付与する変更** — 「どの要素がリンクになるか」と「`to` の値（例: `/game/${sessionId}/agent/${encodeURIComponent(name)}`）」は契約。`render` 後に該当要素の `href` / `getAttribute('href')` を assert するテストを先に書き、RED を確認してから実装する。`encodeURIComponent` の有無・パスの組み立ても assert に含める
- **`useNavigate` による遷移** — クリックで期待ルートへ遷移することを assert する

> 実例: #342 で roster の `Link` 化を「a タグになるだけ」と軽視し、Contract テストの設計をスキップ→実装ごと漏れた。`to` の値はルーティングの契約であり、純粋関数のパースと同格に扱う。
...
カバレッジ確認中にユーザーが見た目の修正・UI調整を始めた場合は、カバレッジ確認を中断し「Playwright 動作確認フェーズに戻りますか？」と提案する。
...
> **⚠️ テキストレポートに 100% カバーのファイルが出ない環境がある**: 一部の環境（vitest/coverage-v8 4.x × Node 22 等）では、テキストレポートのテーブルが**全項目 100% カバーのファイルを行ごと省く**ことがある（`skipFull=false` を明示しても変わらない）。新規追加した小さなファイル（adapter・純粋関数など）が完全カバーだと、テーブルに現れず「カバレッジが出ていない＝未カバー」と誤読しやすい。**テーブルに出ない≠未カバー**。
>
> 対象ファイルの行がテキストレポートに見当たらないときは、推測でツール仕様を語らず **`coverage/coverage-final.json`（machine-readable な一次ソース）を確認する**。各ファイルの `s`（statement 実行回数）の値が全て 1 以上ならカバー済み。`All files` の集計値が正なら計測自体は正しく、テキスト表示だけの問題と切り分けられる。
