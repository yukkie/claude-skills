...
### フロントエンドの設計方針（#338 で確立）

- **データアクセス境界**: fetch URL は `archiveLoader.js` のような専用モジュールに集約する。将来 FastAPI エンドポイントへの差し替えがそのファイルの1行変更で済むように設計する
...
1. contract テストを書く
2. `pytest` / `npm test` で **RED を確認する**（RED にならない場合はアサートが甘い — 見直す）
3. 実装する
4. GREEN を確認する
