...
3. **producer / consumer の全列挙** — スキーマ・フォーマットを変える場合、それを **emit する側（producer）** と **読む側（consumer）** を機械的に全部挙げる。consumer は CLI renderer・JS parser・replay・テストなど複数言語にまたがることが多い（CLI renderer の見落としに注意）。
...
PR 作成後の**区切り**でのみ実行する。作業の途中では絶対に割り込まない。

このセッション中に、ユーザーが示した**言語化できる判断・方法論**（暗黙知）があれば、それを検出してワンクッション挟んで確認する。例: 「3分割のほうが筋がいい」→ producer/consumer/言語の軸で分割を判断する基準、「往復変換があれば淀み」→ スメルの定義、など。

### 動作

1. **検出（内部）**: このセッションでユーザーが下した判断のうち、**判定基準や手順の形に書き下せるもの**を拾う。単なるその場の選択ではなく、*次回も再利用できる方法論*に限る。
2. **ワンクッション確認**: 検出したら、以下のように*静かに差し出す*。割り込まず、PR 完了の区切りで一度だけ聞く:
   ```
   ...
   - {教訓2: ...}
   ```
3. **固着先の選択（y の場合のみ）**: 教訓ごとに行き先を選ぶ。**「今は固めず寝かせる」を常に第一選択肢**に置く（早すぎる言語化は陳腐化・硬直を招くため）:
   - **スキルに追記** — 再利用する方法論（このスキルや他スキルへ）
   - **Issue 化** — ツール構想・改善タスクなど作業を伴うもの（`idd create` フロー or 直接作成）
   - **寝かせる** — まだ熟していない。何もしない
4. **移譲の可視化**: 追記・Issue 化したら「今回 N 個をルール化／Issue 化しました」と明示する。知識が静かに吸われるのではなく、ユーザーが移譲を意識的にコントロールできるようにする。

### 不変条件

- **主導権はユーザー**。AI は検出と提示までで、固着は必ず承認後。
- **割り込まない**。起動は PR 後など区切りのみ。考えている途中で「言語化しましょう」と遮らない。
- **寝かせるを尊重**。「今は言葉にしない」も正しい判断として常に提示する。
