...
承認後、`mcp__gh-mcp__gh_issue_create` ツールで作成する（body の長文 Markdown をシェル経由で渡すと PowerShell のクォート／here-string で事故るため、`gh issue create` を直接呼ばずこのツールを使う）:
```
...
```
返り値 `{ ok, url, stdout, stderr, exit_code }` の `url` が作成された Issue の URL。`ok` が false なら `stderr` を確認する。
...
承認後、`mcp__gh-mcp__gh_issue_edit` ツールで Issue 本文を更新する（長文 body をシェルに渡さないため）:
```
mcp__gh-mcp__gh_issue_edit(
  repo="yukkie/AgentVillage",
  number={番号},
...
| 状況 | 対応 |
|---|---|
| `gh issue view` が失敗 | Issue番号を確認してユーザーに報告。`doc/Ideas.md` の情報だけで続けるか確認する |
