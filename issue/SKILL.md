...
## 横断ルール: 順序制約（順番そのものが仕様）を Issue に明記する

上の「依存」ルールは **成果物の有無**（producer が部品を作る／作らない）を扱う。だがそれとは別に、**成果物はいずれ揃うが、実装する順番に意味がある**ケースがある。これは「依存」では捉えきれない別類型なので、専用に書き戻す。

典型は **「実データ化 → その後で残ったスタブ／ダミーを撤去」** のような工程順だ。撤去 Issue を実データ化 Issue より先にやると、撤去側は「実データ化前の構造」を前提に掃除することになり、撤去漏れが起きる。両 Issue の成果物（撤去対象・実データ）はどちらも存在するので「依存（成果物の有無）」では検知できない。**順番を逆にすると一方の AC が中途半端に達成される**——これが順序制約のシグナルである。

順序制約があると判断したら、関係する各 Issue の本文冒頭（`依存:` の近く）に **実装順序ブロック**を書く:

```
**実装順序（順番そのものが仕様）**:
- 実装順: #A → #B → #C
- 前提状態: #B は「#A 実装後の {対象}（{どんな状態か}）」を前提にする。
- 逆順で実装した場合の注意: #B を #A より先に実装すると、{#A がもたらすはずの状態} が無いまま作業することになり、{起きうる撤去漏れ・取りこぼし} が生じうる。
- 再照合条件: 逆順で進めた場合、後続 Issue 実装時に {前 Issue の対象} が残っていないか／中途半端でないかを再確認する。
```

判定と運用:

- **「依存」か「順序制約」かの見分け**: 後続が前の**成果物（コード・部品・データ）を使う**なら依存。後続が前の**完了後の状態を前提に動く**（順番を逆にすると AC の達成度が変わる）なら順序制約。両方該当することもある（その場合は両方書く）。
- **`issue create` / `split`**: 工程順のある Issue 群を起票・分割するとき、上の実装順序ブロックを各 Issue に織り込む。特に「掃除・撤去・移行・無効化」系の Issue は、その前段（実データ化・新構造導入）との順序を必ず確認する。
- **`issue revise`**: 既存 Issue を点検するとき、関連 Issue 群に工程順があるのに実装順序ブロックが無ければ追加を提案する。さらに **実際に逆順で実装されてしまった形跡**（前 Issue 未完のまま後続が進んだ等）があれば、その事故を Issue に明記して後続の再照合を促す。

> **実例（この対策が生まれた事故）**: #523（game-scoped を実データ化）→ #524（実データ化後に残った stub-only UI / trust を撤去）という工程順だったが、Issue にその順序が書かれていなかった。実際は #524 → #523 の逆順で進み、#524 は実データ化前の構造を前提に掃除したため trust 表示の撤去が中途半端に。結果、#523 側で「触るな」と書いてあった trust を実データ化の都合で撤去せざるを得なくなり、#523 の AC-7 とコードが矛盾した。これは「依存（trust 撤去部品の有無）」の問題ではなく、**順番そのものが仕様**だったのに明記されていなかった問題である。
