...
open / closed の両方を取得して突き合わせる（open と closed を別々に取得して合算する）:
```
mcp__plugin_github_github__list_issues(owner="yukkie", repo="AgentVillage", state="OPEN", perPage=100)
mcp__plugin_github_github__list_issues(owner="yukkie", repo="AgentVillage", state="CLOSED", perPage=100)
```
...
> **SP が大きい（`sp:8` / `sp:13`）Issue は分割を検討する**。分割すると判断したら、その場で `issue_write` を直接呼ばず **`/issue split <番号>` を案内する**。分割は子 Issue を作るだけでなく、**その Issue に依存していた既存 Issue の依存リンクを張り替える**必要があり（依存の双方向再配線）、`issue split` サブコマンドがその張り替えを手順として担保する。

承認後（ラベル操作のみのため CLI を使う）:
```bash
...
承認後に `mcp__plugin_github_github__issue_write` ツールで作成する（手順は `/issue` の C-3 と同じ。Ideas.md への追記・自動クローズ防止の注意も同様）。
...
- 記載がなければ、優先度更新と同時に Issue 本文に追記する（`mcp__plugin_github_github__issue_write(method="update", owner="yukkie", repo="AgentVillage", issue_number={番号}, body="...")`。body 全体を置き換えるため、既存本文＋追記分をまとめて渡す）
...
mcp__plugin_github_github__create_pull_request(
  owner="yukkie",
  repo="AgentVillage",
  head="docs/backlog-refinement-#{N}",
  base="master",
...
| 状況 | 対応 |
|---|---|
| `list_issues` が失敗 | リポジトリ指定を確認してユーザーに報告 |
