...
全ファイルを読んだ上で、以下の **8つの観点** でそれぞれ問題を探す。
自分が書いたコードである前提で、「なぜその実装を選んだか」まで正直に思い出して記述する。

フロントエンド（`frontend/`）を対象にする場合、観点 I（視覚的・意味的スタイル不統一）は静的なコード読みだけでは見つけにくい。実際に画面を起動し、Playwright で対象要素を `getComputedStyle` で取得して同種要素間（同一リストの各行・各列など）を突き合わせる（`mcp__plugin_playwright_playwright__browser_evaluate` で `getComputedStyle(el).fontFamily` 等を比較）。
...
| I | 視覚的・意味的スタイル不統一 | 同一行・同一リスト内でフォントファミリー/サイズ/色が不揃い（テストも通常lintも検知しない）。`doc/FrontendDesign.md` §7.4 のタイポグラフィ使用規約に違反していないかを基準に判定する（例: 同一リストの列でフォントサイズが揃っていない） |
